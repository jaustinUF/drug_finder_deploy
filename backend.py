...
SYSTEM_PROMPT = """
You are MedFind, a consumer drug-info assistant.
- Audience: non-professionals. Use plain language; no medical advice or dosing instructions.
- Do not narrate tool calls or planning. Present only the final answer.
- Prefer: RxTerms for display fields; AllRelated for brand/generic/forms; NDC tools when user supplies an NDC.
- When name lookup fails, try approximate matches and offer 2–5 likely alternatives.
    ...
    async def process_query(self, query: str) -> str:
        ...
        while True:
            ...
            response = self.anthropic.messages.create(
                ...
                messages=self.messages,         # complete conversation, so far
            )
            # print tool used
            try:
                tool_uses = [b for b in getattr(response, "content", []) if getattr(b, "type", None) == "tool_use"]
                if tool_uses:
                    for b in tool_uses:
                        name = getattr(b, "name", "?")
                        # tool_id = getattr(b, "id", "?")
                        # # b.input is usually a dict
                        # keys = list(getattr(b, "input", {}).keys()) if isinstance(getattr(b, "input", {}),
                        #                                  dict) else type(getattr(b, "input", {})).__name__
                        # # print(f"** MODEL_TOOL_USE**\nname: {name} id: {tool_id} args: {keys}")
                        print(f"** Tool name: {name}")
            except Exception as e:
                log.warning("Tool-use printing failed: %s", e)
            # end print

            log.info("model reply types: %s", [getattr(x, "type", "?") for x in response.content])
            # response will be one of these two types
            tool_calls = []
            free_text_chunks = []
            # Collect free text and tool_use blocks
            for item in response.content:
                if item.type == 'text':
                    free_text_chunks.append(item.text or "")    # collect free text.
                elif item.type == 'tool_use':                   # collect tool call request
                    tool_calls.append({'id': item.id, 'name': item.name, 'input': item.input})

            # # Normal exit path: no tool calls → return final text
            if not tool_calls:
                ...
                return final_text                               # exit loop (n)o tools requested)

            # Record assistant turn (with tool_use)             # One or more tools requested
            ...
            # Steering message if suppression triggered
            ...
            # Safety stop after too many loops
            if loop_idx >= MAX_TOOL_LOOPS:
                ...
                # let the model return a graceful-exit message
